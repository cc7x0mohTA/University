...
struct varchain
{
   ...
   {
        size_t length = {c1.repr.size() < c2.repr.size() ? c1.repr.size(): c2.repr.size()};
        
        auto var1 = c1.repr.begin(), var2 = c2.repr.begin();
        
        for( ; var1 != c1.repr.end() && var2 != c2.repr.end(); ++var1, ++var2 )
        {
            if(var1->v == var2->v)
            {
                if(var1->n < var2->n)
                    return -1;
                else if(var1->n > var2->n)
                    return 1;
            }
            else
            {
        ...
        if(var1 != c1.repr.end())   return -1;
        if(var2 != c2.repr.end())   return  1;
        return 0;
   }
