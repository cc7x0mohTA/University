...
\node [draw={black}, fill=black!10, very thick, rectangle, rounded corners, inner sep=12pt, inner ysep=12pt] (box){%
    ...
    Podwójna wieża Hanoi składa się z $2n$ krążków  $n$ różnych rozmiarów po $2$ krążki każdego rozmiaru. W jednym kroku przenosimy dokładnie jeden krążek i nie możemy kłaść większego na mniejszym. Ile kroków potrzebnych jest by przenieść wieżę z pręta A na B, gdy krążki równej długości nie są rozróżnialne?
    \end{minipage}
};
\node[fill={black}, text=white, rounded corners, right=10pt] at (box.north west) {Zadanie 10};
\end{tikzpicture}
\end{center}
