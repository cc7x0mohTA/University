...
\node [draw={black}, fill=black!10, very thick, rectangle, rounded corners, inner sep=12pt, inner ysep=12pt] (box){%
    ...
    Oblicz dla dowolnych liczb $x \in \mathbb{R}$ i $m \in \mathbb{N}$ wyrażenie 
    $$
    \lfloor x/m \rfloor + \lfloor (x+1)/m \rfloor + \ldots + \lfloor (x+m-1)/m \rfloor
    $$

    \end{minipage}
};
\node[fill={black}, text=white, rounded corners, right=10pt] at (box.north west) {Zadanie 2};
\end{tikzpicture}
\end{center}


\begin{proof}
Niech $x = mk + r$, gdzie $k \in \mathbb{N}, r \in [0, m)$.
Oznaczmy sobie $a_i = \lfloor \frac{x+i}{m} \rfloor$.
...
Wtedy suma z zadania jest równa $mk + \lfloor r \rfloor = \lfloor x \rfloor$. 	
\end{proof}
