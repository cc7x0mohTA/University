...
\node [draw={black}, fill=black!10, very thick, rectangle, rounded corners, inner sep=12pt, inner ysep=12pt] (box){%
    ...
    Dla każdej z nastepujących zależności rekurencyjnych określ liczbę warunków początkowych niezbędnych do jednoznaczenego określenia wartości elementów ciągu dla wszystkich $n \in \mathbb{N}$.

    \begin{itemize}
    \item $a_n = na_{n-2}$
    \item $a_n = a_{n-1} + a_{n-3}$
    \item $a_n = 2a_{\lfloor n / 2\rfloor} + n.$
    \end{itemize}

    \end{minipage}
};
\node[fill={black}, text=white, rounded corners, right=10pt] at (box.north west) {Zadanie 3};
\end{tikzpicture}
\end{center}

\begin{rozw}

Zadanie raczej nie wymaga komentarza. 

\begin{itemize}

\item Odp : 2. Potrzebne jest $a_0, a_1$.
\item Odp : 3. Potrzebne jest $a_0, a_1, a_2$.

\item Odp : 0. Potrzebne jest $a_0$, ale zauważmy, że jeśli $a_0 =  2a_{\lfloor 0 / 2\rfloor} + 0 = 2a_0$, to z tego wynika, że $a_0 = 0$.
